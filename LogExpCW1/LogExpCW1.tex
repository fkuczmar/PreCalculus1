...
\item Find an expression $x=c^{-1}(y)$ for the inverse of the function
\[
   y= c(x) = \frac{1}{2}\left( e^x + e^{-x} \right) \, , \, x\geq 0.
\]
Start by making the substitution $u=e^x$ to get a quadratic equation in $u$. Then complete the square to solve for $u$ in terms of $y$. Then substitute back $u=e^x$ to solve the original equation for $x$ in terms of $y$.

\item State the domain of the function $x = c^{-1}(y)$. Use set-builder notation.

\item Verify algebraically that $c^{-1}$ undoes $c$ and that $c$ undoes $c^{-1}$.
...
\item Is the function $g$ one-to-one? Explain.
\begin{freeResponse}
\end{freeResponse}

\item Find an expression for the inverse $x =g^{-1}(y)$ of the function
\[
   y= g(x) = \frac{1}{2}\left( e^x - e^{-x} \right).
\]

Start by making the substitution $u=e^x$ to get a quadratic equation in $u$. Then complete the square as before.

\item Verify algebraically that $g^{-1}$ undoes $g$ and that $g$ undoes $g^{-1}$.
