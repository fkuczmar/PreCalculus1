...
\section{Completing the Square, Part 1}
An important part of this class is developing algebra skills and so we will almost never rely on formulas. For solving equations this generally means not using the quadratic formula. Instead, we'll complete the square.

Here's an example.

\begin{example}  \label{Exmve83r23r}
Solve the equation
\[
    x^2 + 18x -3 = 0.
\]
...
\begin{explanation}
We start by adding $3$ to both sides of the equation to get
\[
   x^2 + 18x = 3.
\]
Next we write $18x$ as $9x + 9x$ and the equation as
\[
   x^2 + 9x + 9x = 3.
\]
The reason for doing this is to interpret the left side as the sum of three areas - one square with dimensions  $x\times x$ and two rectangles, each with dimensions $9\times x$. See the picture below.

\begin{onlineOnly}
    \begin{center}
\desmos{hz4c6f7tio}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/calculator/hz4c6f7tio}{141: Completing the Square 22}
...
The key idea is to look at the two rectangles, each with area $9x$. We know the length of each rectangle is equal to the side length $x$ of the big red square. So the width of each rectangle must be
\[
    \frac{\answer{9x}}{\answer{x}} = 9 .
\]
...
\begin{onlineOnly}
    \begin{center}
\desmos{arhlne3gxu}{900}{600}
\end{center}
\end{onlineOnly}

\href{https://www.desmos.com/calculator/arhlne3gxu}{141: Completing the Square 22B}

This tells us that we should add the additional area $9^2$ to both sides of our last equation
\[
      x^2 + 9x + 9x = 3
\]
to get
\[
   x^2 + 9x + 9x + 9^2 = 3 + 9^2 .
\]
Now the left side is a perfect square. From the last picture we can see the big square (ie. the whole picture) has side length $x+9$. So our equation becomes
\[
   (x + 9)^2 = 84 .
\]
Then 
...
\section{Completing the Square, Part 2}

Here's an example where the leading coefficient is not $1$.
...
The key idea is to look at the two rectangles, each with area $15x$. We know the length of each rectangle is equal to the side length $3x$ of the big red square. So the width of each rectangle must be
\[
...
and
\[
     x = \frac{1}{3}\left( -5 \pm \sqrt{103}  \right) .
\]
...
is
\[
 \left\{ x | x =  \frac{1}{3}\left( -5 \pm \sqrt{103}  \right) \right\} .
\]
\end{explanation}
...
\begin{enumerate}
\item Be lazy. Do not complete the square unless necessary. 

\item Do not use the quadratic formula. Complete the square as in Example 1 above if necessary. Include a picture as in that example.

\item Find all real solutions to any quadratic equations. Write \emph{no real solutions} if there are none. Do \emph{not} give complex solutions.

\item Do \emph{not} use a calculator.
\end{enumerate}

\begin{exercise} \label{EXMf39201}
Let
\[
    g(x) = 23 - x^2 .
\]
...
\begin{exercise} \label{EXMf39202}
Let
\[
    g(x) = 23 - (x-4)^2 .
\]
...
\begin{exercise} \label{ExMfd83r1fl}
Let 
\[
     f(x) = x^2 + 10x + 7.
\]
...
\[
     f(x) = 1.
\]
Do this by completing the square. Include a picture to help with your explanation.

\item Solve the equation
\[
        f(x) = x .
\]
Complete the square as in Example 1 above. Work with fractions, not decimals.
...
\[
     h(x) = 0 
\]
by completing the square as in Example 2 above.
...
\begin{exercise} \label{EXMf392013}
Let
\[
    f(x) = 16x - 7(x+1)^2 .
\]
