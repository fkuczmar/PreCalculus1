...
A common response when asked about the Pythagorean theorem is to say $a^2 + b^2 = c^2$. But this is incomplete without giving the meanings of $a$, $b$, and $c$. They are the lengths of the sides of a \emph{right} triangle. Here $a$ and $b$ are the lengths of the two \emph{legs}. These are the sides that form the right angle and are the two shorter sides in the triangle. The parameter $c$ is the length of the \emph{hypotenuse}, the side opposite the right angle. This is the longest side.

The Pythagorean theorem in this form is really a statement about areas. 



\begin{exploration} \label{EXP1234}
For the most basic interpretation of the Pythagorean theorem we construct squares on the sides of a right triangle.
\begin{onlineOnly}
    \begin{center}
...
\begin{exploration} \label{ESP09332}
But there's nothing special about squares. Any three curves with the same shape will do. For example, we could construct semicircles on the outside of each side like this.
...
\begin{exploration} \label{EXPldfk3}
...
\begin{exploration}  \label{EXPMDF3}

But probably the best choice, the one that let's us see immediately \emph{why} the Pythagorean theorem is true, is to construct three triangles with the same shape as right triangle $\Delta ABC$ having just the right size.
...
Imagine a $10$-km long railroad track secured only at its two ends to the desert floor. Suppose on a hot day the track expands two meters and buckles into a curve. We would like to compute, or at least approximate, the height of the track above the ground in the middle. We really need calculus to determine the shape of the curve and the exact height, but we can get a pretty good approximation by supposing the track bends into two straight segments running from the high point to its endpoints.

\begin{enumerate}
\item Without doing any computation, guess the height of the track at its midpoint. What do you think? \emph{Do not move on without making a guess.}
\begin{freeResponse}
\end{freeResponse}

\item Now before answering this particular question, let's work in general. We'll suppose the railroad track is $L$ km long in its relaxed state and that it expands $\Delta L$ km due to the heat of the desert. 

We want to find  an expression for the height of the height of the track at its midpoint, assuming  the track bends into two straight segments. Do this as follows.

\begin{enumerate}

\item Start by definining one unknown in a complete sentence. Choose a meaningful unknown for the \emph{height}. Include units in your definition.

\item Next find an expression for the height of the height of the track at its midpoint. 

\item Now use algebra to simplify your expression.

\item Finally, write a concluding sentence.

The height of the track at its midpoint is
\[
  h =  \sqrt{\answer{\frac{L \Delta L}{2}} + \frac{\Delta L^2}{4}} \text{ km}.
\]
Type \textbackslash Delta L for $\Delta L$.

\end{enumerate}

\item Use the result of part (b) to answer the original question about a $10$-km long track that expands two meters. Do not use a calcluator. 

\item How accurate was your guess in part (a)? Are you surprised?

\item Now let's suppose (as is the case in reality) that $\Delta L$ is much less than $L$. Then to approximate the track's height in the middle we can ignore the second term under the square root in your expression from part (b)(iv). This gives the approximate height in the middle as
\[
  h \sim  \sqrt{\answer{\frac{L \Delta L}{2}}} \text{ km} .
\]

\begin{enumerate}
\item Check this expression has the correct units.

\item Interpret the expression under the square root geometrically.
\begin{freeResponse}
\end{freeResponse}

\item Now divide both sides of the equation above by $L$ and simplify the right side to get
\[
    \frac{h}{L} \sim \sqrt{\answer{\frac{\Delta L}{2L}}} .
\]

\item In the original question about a $10$-km track that expands two meters, what are the ratios $\Delta L/L$ and $h/L$? 

The ratios are 
\[
   \frac{\Delta L}{L} = \answer{0.02} \%  
\]
and
\[
  \frac{h}{L} \sim \answer{1} \%.
\]

\item Our conclusion is that if the length of a pinned track increases by $\answer{0.02}\%$, it buckles to a height equal to approximately  $\answer{1}\%$ of its original length.

\item What would happen if the length of a pinned track increased by ${0.01}\%$?
\begin{freeResponse}
\end{freeResponse}
...
\item Now let's gather more data by varying the relative change
\[
        k = \frac{\text{change in the length of the string}}{\text{distance between its pinned ends}},
\] 
...
\begin{enumerate}
\item Find a expression for the height.
...
\item When a railroad track expands $50$cm, its buckles to a height of $20$ meters. How high would the buckle have been if

\begin{enumerate}
\item the same track expanded $100$cm instead?

\item a track of the same material twice as long expanded $50$ cm?  $100$ cm?
\end{enumerate}
\end{enumerate}
