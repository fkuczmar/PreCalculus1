...
But probably the best choice, the one that let's us see immediately \emph{why} the Pythagorean theorem is true, would be to construct triangle2 with the same shape as right triangle $\Delta ABC$ and just the right size.
...
Explain how does this picture \emph{proves} the Pythagorean theorem. 
\begin{freeResponse}
\end{freeResponse}
\end{question}
...
\item Now let's gather more data. You can change the relative change
\[
        k = \frac{\text{change in the length of the stretched string}}{\text{distance between its pinned ends}},
\] 
in the length of the string by dragging point above. Choose the values $k=0.2, 0.3, 0.4, 0.5$ to complete the table in Line 1.
