...
\begin{explanation}
This example is just like the previous one, but it works out much easier (it's the one we did in class). As before, the center of such a circle must be

\pskip

(a) equidistant from $A$ and $B$, and 

\pskip
 
(b) a distance of $\sqrt{26}$ from $A$. 

\pskip

With $P(x,y)$ as the center, the first condition gives
\[
