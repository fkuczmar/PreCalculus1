...
One approach is to think about the action of $f$. The function $f$ first multiplies the input by $4$ and then subtracts $3$ from the result. To undo this process, that is, to express the input $x$ in terms of the output $y$, we first undo the second action (subtracting $3$) by adding $3$. Then we undo the first action (multiplying by $4$) by multiplying by the reciprocal $1/4$. The result is that
\[
...
\[
       y = f^{-1}(x) =  \frac{1}{4}\left( y+3 \right) .
\]

To show algebraically that $f$ and $f^{-1}$ are actually inverses, we need to show that $f^{-1}$ undoes $f$ and that $f$ undoes $f^{-1}$. For the first, we simplify the composition $f^{-1}(f(x))$:
\begin{align*}
        f^{-1}(f(x)) &= f^{-1}(4x-3) \\
...
Use the method of the previous example to find an expression for the inverse of the function 
\[
