...
\item Let  $T=u(h)$ be the function that as an input a height (in meters) and returns as an output the time (in seconds past noon) when the rock was at that height and on its way up. 

Use the graph of the function $f$ above to approximate each of the following expressions.
...
\item Find an expression for the function $T=u(h)$. Include its domain.
